...
\begin{figure}[!h]
  ...
  \includegraphics[width=0.32\textwidth, height= 0.32\textwidth]{pixel/hdi_plate}
  \includegraphics[width=0.32\textwidth, height= 0.32\textwidth]{pixel/hdi_stencil}
  \caption[BBM/HDI plate]{Left: BBM/HDI plate with a mock module that reproduce the BBM features. Center: the pockets in the top and bottom sides accommodate the module holders. Right: bare HDIs showing the alignment provided by the stencil.}\label{fig:bbm_hdi_plates}
\end{figure}

Four chucks are used to accommodate sixteen BBMs (four per plate); the holes in the BBM/HDI plate (see Figure \ref{fig:bbm_hdi_plates}) are intended to hold the BBM/HDI safely fixed to the plate by the action of the vacuum, while the stencil (100 $\mu$m in thickness) allows for a very accurate positioning of the BBM/HDI.     

One chuck is dedicated to accommodate four HDIs. Although BBM/HDI plates can be used indistinctly, the HDI chuck have four independent pockets instead of only a big one, in order to enable the liberation of one HDI at a time; hence, it is connected to 4 vacuum lines. That is not required for the BBMs because they are no moved from their original location.      
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.48\textwidth]{pixel/glue_reservoir}
  \caption[Glue reservoir plate]{Glue reservoir plate. The four pockets are 100 $\mu$m deep. }\label{fig:glue_reservoir}
\end{figure}

The remaining two chucks house the \ti{stamp plate} and the \ti{weight plate}.   These tools are a set of custom made tools, all produced in house by the UNL Physics department machine shop; 

\begin{figure}[!h]
  \centering
  \includegraphics[width=0.48\textwidth]{pixel/tools}
  \caption[Bare chuck and Stamp chuck]{Left: Chuck detailed internal view. Right: Stamp chuck housing the stamp tools. }\label{fig:tools}
\end{figure}




Weights: brass for weight and stainless steel for blades (have polished edges to protect bond pads from scratching) and springs
Stamps: brass for weight, rubber stamp
