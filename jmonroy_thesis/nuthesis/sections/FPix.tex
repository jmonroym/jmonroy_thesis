...
The gluing routine workflow is shown in Figure\ref{fig:glue_workflow}; it was implemented in a LabVIEW program (\ti{Gluing\_main.vi}) that perform the different steps. The Front panel of the gluing routine is shown in Figure \ref{fig:gluing_front_main}. The module gluing sequence begins by manually placing pre-tested, BBMs, HDIs and tools on their chucks. Once the parts in place, and the program is run, the BBM and HDI identification information (serial numbers) is collected\footnote{An batch numbering strategy was designed in order to identify the modules intrernally at UNL.} in the first step as well as the configuration of the gantry table, \ie, the BBM/HDI slots and tools to be used so that the vacuum system is properly activated. 
\begin{landscape}
\hspace{-3cm}
\begin{figure}[h]
\begin{center}
\includegraphics[width=25cm,height=20cm]{pixel/gluing_front_main.png}
