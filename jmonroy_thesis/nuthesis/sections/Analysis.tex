...
In the signal region of this analysis, non-prompt leptons are predominantly produced in \ttbar events, with a much smaller contribution from Drell-Yan production; therefore, the control region, also know as the \ti{application region}, is defined by modifying the event selection criteria in such a way that most of the events after selection are \ttbar events and thus the misidentification rate is increased. The application regions for electrons and muons are defined by the \ti{fakeable} object definitions in Tables ~\ref{tab:muonIDs} and~\ref{tab:eleIDs}. Since the fakeable definition is a loosened version of the tight definition, in the context of fake rates, the fakeable definition becomes the loose selection. 
...
Data (35.9 fb$^{-1}$)         & \multicolumn{1}{l}{127}&\multicolumn{1}{l}{280} & \multicolumn{1}{l}{525} \\\hline
\end{tabular}
...
\end{tabular}
\caption[BDTG input variables.]{BDTG input variables. First section lists variables related to forward jet activity; second section lists variables related to lepton kinematics, and third section lists variables related to jet multiplicities.}
\label{tab:bdtinputs}
\end{table}
...
\caption[BDTG classifier Input variables distributions.]{Distributions of the BDTG classifier input variables for signal discrimination in the $3l$ channel.} 
\label{fig:input_vars_3l}
\end{figure}    

Given that the forward activity is a feature of the \tHq process, the location of the leptons and jets with respect to the forward jet would provide a good way to differentiate signal and background events. Looking at the separation in \etac between the forward jet and the hardest \bjet, for signal events the latter will be located more centrally the more harder it is  is more evident in other plots like in the plot of the maximum $|\eta|$ of the jets in the event, while the former will be in the forward region, thus, their separation will increase; for background events, jets tend to be more central and their separation is smaller compared to signal events. Similar argument works for the \etac separation between the forward jet and the second hardest \bjet and for the separation between the forward jet and the closest lepton.  

The discrimination power  (row two, column three) given the presence of the forward jet in the signal events; for background events, jets tend to be in the central region of the detector.

Variables related to the lepton kinematics encode the distinctive behavior of leptons in signal events and background events. All the input variables have some discrimination power, however, that power is bigger for some of them; for instance, the third lepton \pt plot (top left in Figure~\ref{fig:input_vars_3l}) shows some discrimination power against WZ and VVV backgrounds for which there is a peak around 30 GeV while the \tHq peak around 18 GeV, \ie, the third lepton in those background processes tends to emerge with a higher momentum than the one from the \tHq process; although this variable is powerful in the discrimination against only two backgrounds, the final discriminator will profit from it.

In the case of the total lepton charge, it is symmetrically distributed for \ttbar (fakes) events while for signal events it is expected to have more positive charge in the final state in the final state because the proton have more $u$ quarks that $d$ quarks, therefore, more W$^+$ bosons and then more positive leptons. Again, this variable is very powerful for discriminating against \ttbar but not that powerful against other backgrounds.    

A similar situation can be seen in the plot for the number of jets; \ttW, \ttZ and \ttH processes tend to have more jets compared to the \tHq process (see Figure \ref{fig:ttbar_sign} for instance) due to initial state radiation or other radiative processes. 
...
In order to determine the systematic uncertainties associated with the fake rates, the BDTG classifier output shapes from a pure MC estimation of fake leptons (in \ttbar) and from the application of fake-rates as measured in QCD MC, applied in \ttbar\ MC events, are compared. The difference in the resulting normalization and output shapes, for both trainings vs. \ttbar\ and vs. \ttV, are estimated and propagated to the fit as normalization and shape variations; Figures ~\ref{fig:frclosure_2lss_ee} and ~\ref{fig:frclosure_3l_elfake} show the results of these closure tests.
...
Finally, the best fit values of the combined \tH + \ttH cros sections times the BR as a function of the CP-mixing angle, are shown in Figure~\ref{fig:bf_r}. 
