...
\multicolumn{1}{c@{\qquad}}{} & \multicolumn{1}{c@{\qquad}}{$3\ell$} & \multicolumn{1}{c}{$\mumu$} & \multicolumn{1}{c@{\qquad}}{$e\mu$} & \multicolumn{1}{c}{$ee$} \\ \hline
...
The number of events that pass both the loose and tight selections, divided by the number of events that pass the loose selection but fail the tight one corresponds to the \ti{fake factor/fake rate}. The measurement of the fake factor is made using two background dominated data samples collected with dedicated triggers (subtracting the residual prompt lepton contribution using MC) as a function of \pt and |\etac| and separately for muons and electrons:

\begin{itemize}
\item A sample dominated by QCD multijet events, collected using single lepton triggers at relatively high \pt thresholds is used to extract ratios for lepton candidates with \pt above 30 GeV.
